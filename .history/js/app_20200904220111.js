...
const getWeatherContent = (arr) => {
  ...
  let mappedArr = arr.map(data => {
    ...
    return `<div class="col-10 col-md-6 m-auto col-lg-4 py-2">
                <div class="card" >
                    <div class="card-body">
                        <h5 class="card-title" id="name">${data.name}</h5>
                        <h6 class="card-subtitle mb-2 text-muted">Lat: <span id="lat" class="badge badge-info">${data.coord.lat}</span>, Lng: <span id="lng" class="badge badge-info">${data.colon}</span> </h6>
...
window.addEventListener("DOMContentLoaded", getWeatherContent);
